...
function showPay(){var sel=$('sale-client');if(!sel)return;sel.innerHTML='<option value="">اختر العميل...</option>';for(var i=0;i<db.clients.length;i++){var c=db.clients[i];sel.innerHTML+='<option value="'+c.id+'">'+c.name+(c.totalDebt?' — دين: '+fmt(c.totalDebt):'')+' </option>';}document.querySelectorAll('.pay-chip').forEach(function(ch){ch.classList.remove('sel');});var first=document.querySelectorAll('.pay-chip')[0];if(first)first.classList.add('sel');var pm=$('pay-method');if(pm)pm.value='cash';var ci=$('cli-sel-wrap');if(ci)ci.style.display='none';var pw=$('partial-pay-wrap');if(pw)pw.style.display='none';var cw=$('cash-change-wrap');if(cw)cw.style.display='block';var dw=$('cli-debt-warn');if(dw)dw.style.display='none';}
function selPay(method,el){
  ...
  if($('cash-given'))$('cash-given').value='';
