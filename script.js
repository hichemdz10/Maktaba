        ...
        .then(function(stream) {
            ...
            camLoop = setInterval(function() {
                ...
                camDetector.detect(video).then(function(barcodes) {
                    ...
                    if (!item) {
                        beep(false);
                        stopSplitCamera(); // إيقاف الكاميرا مؤقتاً لفتح نافذة الإضافة
                        openQuickAdd(code);
...
function openQuickAdd(barcode) {
    ...
    document.getElementById('qa_cancel').onclick = function() { 
        ...
        // إعادة تشغيل الكاميرا إذا كانت مفتوحة قبل فتح النافذة
        if (document.getElementById('splitCameraContainer') && document.getElementById('splitCameraContainer').style.display !== 'none') {
            startSplitCamera();
        }
    };
    document.getElementById('qa_save').onclick = function() {
        ...
        // إعادة رسم السلة في وضع الانقسام إذا كان نشطاً
        if (document.getElementById('splitCameraContainer') && document.getElementById('splitCameraContainer').style.display !== 'none') {
            refreshSplitCart();
...
function addToCartHome(id) {
    ...
    beep(true);
    toast("🛒 " + item.n);
    // تحديث السلة في وضع الانقسام إذا كان نشطاً
    if (document.getElementById('splitCameraContainer') && document.getElementById('splitCameraContainer').style.display !== 'none') {
        refreshSplitCart();
    } else {
        // تحديث السلة الجانبية العالمية
...
function printReceipt() {
    ...
    var rows = S.cart.map(function(it) {
        ...
        return '骨<td style="text-align:right;padding:6px 4px;font-size:13px;border-bottom:1px dashed #e0e0e0">'+esc(it.n)+'</td><td style="text-align:center;padding:6px 4px;font-size:13px;border-bottom:1px dashed #e0e0e0;color:#555">'+it.q+' × '+p+'</td><td style="text-align:left;padding:6px 4px;font-size:13px;border-bottom:1px dashed #e0e0e0;font-weight:700;color:#0277bd">'+it.sum+'</td>';
    }).join('');
...
function renderHome() {
    ...
    var splitView = '';
    if (S.cameraActiveInHome) {
        splitView = '<div id="splitViewContainer" style="display:flex;flex-direction:column;height:100%;gap:12px;">'+
            '<div id="splitCameraContainer" style="flex:4; background:#000; border-radius:20px; overflow:hidden; position:relative; min-height:200px;">'+
                '<video id="splitCameraVideo" autoplay playsinline style="width:100%; height:100%; object-fit:cover"></video>'+
            ...
            '</div>'+
            '<div id="splitCartContainer" style="flex:6; background:#f5f7ff; border-radius:20px; padding:13px; border:2px solid #dde4ff; display:flex; flex-direction:column; overflow-y:auto;">'+
