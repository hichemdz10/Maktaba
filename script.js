        ...
        .then(function(stream) {
            ...
            camLoop = setInterval(function() {
                ...
                camDetector.detect(video).then(function(barcodes) {
                    ...
                    // تحديث السلة فقط (بدون إغلاق)
                    if (S.tab === 'home') {
                        var cartPanel = document.getElementById('cartPanelContent');
                        if (cartPanel) cartPanel.innerHTML = renderCartPanelContent(refreshCartPanel);
                        bindCartEvents(refreshCartPanel);
                    } else {
                        render();
                    }
...
function openQuickAdd(barcode) {
    ...
    document.getElementById('qa_cancel').onclick = function() { ov.style.display = 'none'; };
...
function refreshCartPanel() {
    ...
    if (container) {
        container.innerHTML = renderCartPanelContent(refreshCartPanel);
        bindCartEvents(refreshCartPanel);
    }
}
...
function renderHome() {
    ...
    '</div>';
    // Split view: camera on top, cart below
    var cameraSection = '<div id="splitCameraContainer" style="display:none; margin-bottom:12px; border-radius:20px; overflow:hidden; background:#000; position:relative">'+
        '<video id="splitCameraVideo" autoplay playsinline style="width:100%; height:200px; object-fit:cover; display:block"></video>'+
    ...
    '</div>';
    var cartSection = '<div class="cart-sidebar" id="cartPanel" style="background:#f5f7ff;border-radius:16px;padding:13px;border:2px solid #dde4ff;display:flex;flex-direction:column;box-shadow:0 4px 20px rgba(0,0,0,.09);margin-top:8px">'+
        ...
        '<div id="cartPanelContent">'+renderCartPanelContent(refreshCartPanel)+'</div>'+
    '</div>';
    return '<div class="cart-overlay" id="cartOverlay" onclick="closeCart()"></div>'+
        ...
        '<div>'+
            cameraSection+
            '<div id="homeMainContent">'+rightPanel+'</div>'+
        '</div>'+
        cartSection+
        '</div>'+
        '<script>'+
            'document.getElementById("camBtn").onclick = function() {'+
                'var container = document.getElementById("splitCameraContainer");'+
            ...
            'document.getElementById("closeSplitCamera").onclick = function() {'+
                'stopSplitCamera();'+
                'document.getElementById("splitCameraContainer").style.display = "none";'+
            '};'+
        '<\/script>';
}
...
// ===== STOCK (modified: add button after profit) =====
function addToCartFromStock(id) {
    ...
    if (S.tab === 'home') refreshCartPanel();
    else render();
}
...
function render() {
    ...
    var cartPanelGlobal = '<div class="cart-overlay" id="cartOverlay" onclick="closeCart()"></div>'+
        '<div class="cart-sidebar" id="cartPanel" style="background:#f5f7ff;border-radius:16px;padding:13px;border:2px solid #dde4ff;display:flex;flex-direction:column;box-shadow:0 4px 20px rgba(0,0,0,.09);position:fixed;top:0;right:0;bottom:0;width:360px;z-index:1000;transform:translateX(100%);transition:transform .35s cubic-bezier(.4,0,.2,1)">'+
            ...
            '<div id="cartPanelContent">'+renderCartPanelContent(refreshCartPanel)+'</div>'+
        '</div>';
    document.getElementById('root').innerHTML = '<div class="app">'+renderHeader()+'<div class="tab-content">'+tabContent+'</div>'+cartPanelGlobal+'</div>';
    if(!clockTimer){
    ...
    // ربط أحداث السلة بعد إعادة الرسم
    bindCartEvents(refreshCartPanel);
}
