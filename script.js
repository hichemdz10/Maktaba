...
function scheduleMidnight() {
    ...
    midnightTimer = setTimeout(function() {
        // البيانات لا تُمسح — فقط يُصفّر اليوم تلقائياً لأن tSales() تفلتر حسب التاريخ
        render();
        scheduleMidnight();
    }, ms);
}
...
function openCamera() {
    ...
    var ov = document.getElementById('camModal');
    if (!ov) {
        ov = document.createElement('div');
        ov.id = 'camModal';
        document.body.appendChild(ov);
    }
    ov.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;background:#000;z-index:5000;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:16px;box-sizing:border-box';
    ov.innerHTML = '<div style="color:#29b6f6;font-size:22px;font-weight:800;font-family:Tajawal,Arial">📷 مسح الباركود</div>' +
        '<div style="width:100%;max-width:480px"><video id="camVideo" autoplay playsinline style="width:100%;border-radius:16px;display:block;box-shadow:0 0 30px rgba(41,182,246,.3)"></video></div>' +
        '<div id="camStatus" style="color:#b3e5fc;font-size:16px;font-weight:700;font-family:Tajawal,Arial;text-align:center">جاري تشغيل الكاميرا...</div>' +
        '<div id="camResult" style="min-height:42px;text-align:center"></div>' +
        '<button id="camCloseBtn" style="background:linear-gradient(135deg,#c62828,#e53935);color:#fff;border:none;border-radius:14px;padding:16px 44px;font-size:17px;font-weight:800;cursor:pointer;font-family:Tajawal,Arial">✕ إغلاق</button>';
    ...
    navigator.mediaDevices.getUserMedia({
        ...
        document.getElementById('camStatus').textContent = 'وجّه الكاميرا نحو الباركود 📦';
        ...
        camLoop = setInterval(function() {
            ...
            camDetector.detect(video).then(function(barcodes) {
                ...
                lastScanned = code;
                scanCooldown = true;
                var item = S.stock.find(function(s) {
                    return s.barcode === code;
                });
                ...
                if (item.q <= 0) {
                    beep(false);
                    if (resEl) resEl.innerHTML = '<span style="background:#e65100;color:#fff;border-radius:12px;padding:8px 18px;font-size:15px;font-weight:700">نفذ: ' + esc(item.n) + '</span>';
                    setTimeout(function() {
                        scanCooldown = false;
                        lastScanned = "";
                    }, 2000);
                ...
                beep(true);
                if (resEl) resEl.innerHTML = '<span style="background:#1b5e20;color:#fff;border-radius:12px;padding:8px 18px;font-size:16px;font-weight:700">✅ ' + esc(item.n) + ' — ' + item.p + ' دج</span>';
        ...
        var msg = 'لا يمكن الوصول للكاميرا';
        if (err.name === 'NotAllowedError') msg = 'اضغط السماح للكاميرا';
        else if (err.name === 'NotFoundError') msg = 'لا توجد كاميرا';
...
function showCamUnsupported() {
    var ov = document.getElementById('camModal');
    if (!ov) {
        ov = document.createElement('div');
        ov.id = 'camModal';
        ov.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.85);z-index:5000;display:flex;align-items:center;justify-content:center;padding:24px';
        document.body.appendChild(ov);
    }
    ov.innerHTML = '<div style="background:#fff;border-radius:20px;padding:32px 24px;max-width:380px;width:100%;text-align:center"><div style="font-size:54px;margin-bottom:14px">📷</div><h3 style="color:#c62828;margin:0 0 12px;font-size:19px;font-family:Tajawal,Arial">غير متاح في هذا المتصفح</h3><p style="color:#555;font-size:15px;line-height:1.7;margin-bottom:18px">استخدم <b>Chrome</b> أو <b>Edge</b></p><button id="camCloseUnsup" style="background:#0277bd;color:#fff;border:none;border-radius:12px;padding:13px 32px;font-size:16px;font-weight:700;cursor:pointer;font-family:Tajawal,Arial;width:100%">حسناً</button></div>';
    ov.style.display = 'flex';
    document.getElementById('camCloseUnsup').onclick = function() {
...
function openQuickAdd(barcode) {
    ...
    var catOpts = CATS.filter(function(x) {
        return x !== "الكل";
    }).map(function(c) {
        return '<option value="' + c + '">' + CI[c] + ' ' + c + '</option>';
    }).join('');
    ...
    document.getElementById('qa_save').onclick = function() {
        ...
        if (!n) {
            toast("أدخل اسم المنتج", "e");
            return;
        }
        if (!p) {
            toast("أدخل سعر البيع", "e");
            return;
        }
        if (!c) {
            toast("أدخل سعر الشراء", "e");
            return;
        }
        if (c >= p) {
            toast("سعر الشراء يجب أن يكون أقل", "e");
            return;
        }
        if (q === 0 && wq === 0) {
            toast("أدخل الكمية", "e");
        ...
        var nId = S.stock.length > 0 ? Math.max.apply(null, S.stock.map(function(s) {
            return s.id;
        })) + 1 : 1;
    ...
    setTimeout(function() {
        var el = document.getElementById('qa_n');
        if (el) el.focus();
    }, 150);
}
...
function openManualCart() {
    ...
    var items = S.stock.filter(function(s) {
        return s.q > 0;
    });
    var cats = CATS.filter(function(x) {
        return x !== "الكل";
    });
    ...
    ov.style.display = 'flex';
    setTimeout(function() {
        var el = document.getElementById('manSrch');
        if (el) el.focus();
    }, 150);
}
...
function renderManualList(items, srch) {
    var filtered = items.filter(function(s) {
        return !srch || nAr(s.n).includes(nAr(srch));
    });
    ...
    return filtered.map(function(item) {
        ...
        var inCart = S.cart.find(function(c) {
            return c.id === item.id;
        });
        return '<div style="display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border-radius:13px;border:1.5px solid ' + col + '22;background:' + (inCart ? col + '11' : '#fafafa') + ';margin-bottom:8px">' +
            ...
            '<div>' +
            '<div style="font-size:15px;font-weight:700;color:#1a2535">' + esc(item.n) + '</div>' +
            '<div style="font-size:13px;color:' + col + ';font-weight:700">' + item.p + ' دج <span style="color:#aaa;font-weight:400">· متبقي: ' + item.q + '</span></div>' +
            '</div>' +
            '</div>' +
...
function remFromCart(id) {
    ...
    if (item) item.q += ci.q;
    S.cart = S.cart.filter(function(c) {
        return c.id !== id;
    });
    save();
}
...
function changeCartQty(id, delta) {
    if (delta > 0) {
        ...
        if (!ci) return;
        var it = S.stock.find(function(s) {
            return s.id === id;
        });
        if (it) it.q++;
        ci.q--;
...
function saveSale(client) {
    ...
    S.sales.push(sale);
    if (client) {
        var cl = S.clients.find(function(c) {
...
function cancelSale(sid) {
    var sale = S.sales.find(function(s) {
        return s.id === sid;
    });
    if (!sale) return;
    sale.items.forEach(function(it) {
        var st = S.stock.find(function(s) {
            return s.id === it.id;
        });
        if (st) st.q += it.q;
    });
    if (sale.client) {
        var cl = S.clients.find(function(c) {
            return c.id === sale.client.id;
        });
        if (cl) cl.debt = Math.max(0, cl.debt - sale.total);
    }
    S.sales = S.sales.filter(function(s) {
        return s.id !== sid;
    });
    save();
    render();
...
function transferToShop(id, qty) {
    ...
    if (qty > item.wq) {
        toast("المستودع لا يحتوي إلا على " + item.wq, "e");
        return;
    }
    item.q += qty;
    item.wq -= qty;
...
function addStock() {
    var f = S.form;
    if (!f.n || !+f.p || !+f.c) {
        toast("ملء جميع الحقول", "e");
        return;
    }
    if (+f.c >= +f.p) {
        toast("سعر الشراء أكبر", "e");
        return;
    }
    ...
    var nn = nAr(f.n.trim());
    var idx = S.stock.findIndex(function(s) {
        return nAr(s.n) === nn;
    });
    if (idx >= 0) {
        S.stock[idx].q += shopQ;
...
function delStock(id) {
    S.stock = S.stock.filter(function(s) {
        return s.id !== id;
    });
    save();
    render();
...
function addClient() {
    ...
    S.clients.push({
        ...
        name: S.cForm.name,
        phone: S.cForm.phone || "",
        debt: parseFloat(S.cForm.debt) || 0
    });
    S.cForm = {
        name: "",
        phone: "",
...
function addExp() {
    if (!S.eForm.lbl || !+S.eForm.amt) {
        toast("أدخل البيانات", "e");
        return;
    }
    S.expenses.push({
        id: Date.now(),
...
function delPrint(id) {
    S.pjobs = S.pjobs.filter(function(p) {
        return p.id !== id;
    });
    save();
    render();
...
function delFlixy(id) {
    S.flixy = S.flixy.filter(function(f) {
        return f.id !== id;
    });
    save();
    render();
...
function expCSV(type) {
    ...
    if (type === "sales") {
        ...
        rows = S.sales.map(function(s) {
            return [new Date(s.date).toLocaleString("ar-DZ"), (s.client && s.client.name) || "نقدي", s.items.map(function(i) {
                return i.n + "x" + i.q;
            }).join("|"), s.raw, s.discount, s.total, s.profit];
        });
        fn = "مبيعات.csv";
    } else if (type === "exp") {
        headers = ["التاريخ", "الوصف", "الفئة", "المبلغ"];
        rows = S.expenses.map(function(e) {
            return [new Date(e.date).toLocaleString("ar-DZ"), e.label, e.cat, e.amount];
        });
        fn = "مصاريف.csv";
    } else {
        headers = ["الصنف", "الفئة", "بيع", "شراء", "ربح", "باركود", "المحل", "المستودع"];
        rows = S.stock.map(function(s) {
            return [s.n, s.cat, s.p, s.c, s.p - s.c, s.barcode || "", s.q, s.wq || 0];
        });
        fn = "مخزون.csv";
    }
...
function printReceipt() {
    ...
    var rows = S.cart.map(function(it) {
        ...
        return '<tr><td style="text-align:right;padding:6px 4px;font-size:13px;border-bottom:1px dashed #e0e0e0">' + esc(it.n) + '</td><td style="text-align:center;padding:6px 4px;font-size:13px;border-bottom:1px dashed #e0e0e0;color:#555">' + it.q + ' × ' + p + '</td><td style="text-align:left;padding:6px 4px;font-size:13px;border-bottom:1px dashed #e0e0e0;font-weight:700;color:#0277bd">' + it.sum + '</td></tr>';
    }).join('');
    var html = '<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><title>فاتورة #' + invoiceNum + '</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Tajawal,Arial,sans-serif;font-size:13px;direction:rtl;width:80mm;margin:0 auto;padding:4mm 3mm;color:#000}@media print{body{width:80mm;margin:0;padding:3mm 2mm}@page{size:80mm auto;margin:0}}.center{text-align:center}.dash{border:none;border-top:1px dashed #999;margin:6px 0}.solid{border:none;border-top:2px solid #000;margin:6px 0}table{width:100%;border-collapse:collapse}</style></head><body>' +
        ...
        '<div style="text-align:center;font-size:11px;color:#666;padding-bottom:4px">' + n.toLocaleTimeString("ar-DZ", {
            hour: "2-digit",
            minute: "2-digit",
            hour12: false
        }) + '</div><hr class="dash">' +
...
function renderTabs() {
    ...
    return '<div class="tabs">' + tabKeys.map(function(k) {
        var t = TAB_STYLES[k];
        var active = S.tab === k;
        var badge = (k === "stock" && low > 0) ? '<span style="position:absolute;top:-5px;right:-5px;background:#f44336;color:#fff;border-radius:50%;min-width:18px;height:18px;font-size:11px;font-weight:800;display:inline-flex;align-items:center;justify-content:center;border:2px solid #0d1b2a">' + low + '</span>' : '';
        return '<button class="tab-btn" onclick="S.tab=\'' + k + '\';save();render()" style="' +
            'background:' + (active ? t.activeBg : t.bg) + ';' +
            'color:' + (active ? '#fff' : t.color) + ';' +
            'border-bottom:' + (active ? '4px solid #fff' : '4px solid transparent') + ';' +
            'box-shadow:' + (active ? '0 6px 18px rgba(0,0,0,.3)' : '0 2px 8px rgba(0,0,0,.1)') + ';' +
...
// ===== CART PANEL =====
function renderCartPanel(refreshFn) {
    ...
    var payRow = S.cart.length > 0 ?
        ...
        (S.clients.length > 0 ?
            ...
            S.clients.map(function(cl) {
                return '<button data-client="' + cl.id + '" data-cname="' + esc(cl.name) + '" style="flex:1;background:rgba(255,255,255,.92);color:#1b5e20;border:none;border-radius:10px;padding:9px;cursor:pointer;font-size:13px;font-family:Tajawal,Arial;font-weight:700">' + esc(cl.name) + (cl.debt > 0 ? '<div style="font-size:11px;color:#c62828">' + fmt(cl.debt) + ' دج</div>' : '') + '</button>';
            }).join('') +
            '</div></details>' : '') +
        '</div>' : '';
    ...
    return '<div style="display:flex;flex-direction:column;height:100%;gap:8px">' +
        ...
        '<div style="color:#fff;font-weight:800;font-size:16px">فاتورة البيع</div>' +
        '</div>' +
        (S.cart.length > 0 ?
            '<div style="display:flex;gap:6px">' +
            '<span style="background:rgba(255,255,255,.15);color:#fff;border-radius:20px;padding:3px 11px;font-size:13px;font-weight:700">' + S.cart.length + ' صنف</span>' +
            '<span style="background:#0277bd;color:#fff;border-radius:20px;padding:3px 11px;font-size:13px;font-weight:700">' + cTotal() + ' دج</span>' +
            '</div>' : '<span style="color:rgba(255,255,255,.4);font-size:13px">فارغة</span>') +
        '</div>' +
...
function bindCartEvents(refreshFn) {
    ...
    document.querySelectorAll('[data-inc]').forEach(function(btn) {
        btn.onclick = function() {
            changeCartQty(+btn.dataset.inc, 1);
            save();
            refreshFn();
        };
    });
    document.querySelectorAll('[data-dec]').forEach(function(btn) {
        btn.onclick = function() {
            changeCartQty(+btn.dataset.dec, -1);
            save();
            refreshFn();
        };
    });
    document.querySelectorAll('[data-price]').forEach(function(inp) {
        inp.oninput = function() {
            updateCartItemPrice(+inp.dataset.price, +inp.value);
            refreshFn();
        };
    });
    document.querySelectorAll('[data-disc]').forEach(function(btn) {
        btn.onclick = function() {
            S.disc = +btn.dataset.disc;
            refreshFn();
        };
    });
    ...
    document.querySelectorAll('[data-client]').forEach(function(btn) {
        btn.onclick = function() {
            if (confirm("على حساب " + btn.dataset.cname + "؟")) saveSale({
                id: +btn.dataset.client,
                name: btn.dataset.cname
            });
        };
    });
}
...
function renderHome() {
    var stats = [{
            ...
            l: "مبيعات اليوم",
            v: H(fmt(tST()) + " دج"),
            g: "linear-gradient(135deg,#0d47a1,#1565c0,#0277bd)"
        },
        {
            ic: "📈",
            l: "أرباح اليوم",
            v: H(fmt(tPR()) + " دج"),
            g: "linear-gradient(135deg,#1b5e20,#2e7d32,#388e3c)"
        },
        {
            ic: "🖨️",
            l: "طباعة اليوم",
            v: H(fmt(tPT()) + " دج"),
            g: "linear-gradient(135deg,#004d40,#00695c,#00897b)"
        },
        {
            ic: "📱",
            l: "Flixy اليوم",
            v: H(fmt(tFT()) + " دج"),
            g: "linear-gradient(135deg,#1a237e,#283593,#3949ab)"
        },
        {
            ic: "🏪",
            l: "قيمة المحل",
            v: H(fmt(shopVal()) + " دج"),
            g: "linear-gradient(135deg,#bf360c,#d84315,#e64a19)"
        },
        {
            ic: "📋",
            l: "إجمالي الديون",
            v: H(fmt(tDebt()) + " دج"),
            g: tDebt() > 0 ? "linear-gradient(135deg,#b71c1c,#c62828,#e53935)" : "linear-gradient(135deg,#1b5e20,#2e7d32,#43a047)"
        }
    ];
    var statsHTML = '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin-bottom:18px">' +
        stats.map(function(s) {
            return '<div style="background:' + s.g + ';border-radius:16px;padding:15px 9px;text-align:center;box-shadow:0 5px 15px rgba(0,0,0,.25);transition:transform .2s" onmouseover="this.style.transform=\'translateY(-3px)\'" onmouseout="this.style.transform=\'none\'">' +
    ...
    var catBtns = '<div style="margin-bottom:8px;font-size:15px;font-weight:800;color:#1a2535">🗂️ تصفح حسب الفئة</div>' +
        ...
        CATS.filter(function(x) {
            return x !== "الكل";
        }).map(function(cat) {
            var col = CC[cat];
            var img = CAT_IMG[cat] || "";
            var count = S.stock.filter(function(s) {
                return s.cat === cat;
            }).length;
        ...
        (lastSales.length === 0 ? '<p style="text-align:center;padding:20px;color:#aaa;font-size:15px">لا توجد عمليات بعد</p>' :
            lastSales.map(function(s, i) {
                return '<div style="background:#fff;border:2px solid #e8eeff;border-right:5px solid #0277bd;border-radius:13px;padding:11px 13px;margin-bottom:8px">' +
                    ...
                    '</div>' +
                    '<div style="display:flex;flex-wrap:wrap;gap:4px">' + s.items.map(function(it) {
                        return '<span style="background:#e8f4fd;color:#01579b;border-radius:7px;padding:3px 9px;font-size:12px">' + esc(it.n) + ' ×' + it.q + '</span>';
                    }).join('') + '</div>' +
                    '</div>';
            }).join('')) +
        '</div>';

    var rightPanel = '<div style="display:flex;flex-direction:column;gap:12px">' +
    ...
    return '<div class="cart-overlay" id="cartOverlay" onclick="closeCart()"></div>' +
        ...
        '<button onclick="closeCart()" id="closeCartBtn" style="display:none;background:linear-gradient(135deg,#c62828,#e53935);color:#fff;border:none;border-radius:11px;padding:10px;font-size:15px;font-weight:700;font-family:Tajawal,Arial;margin-bottom:10px;cursor:pointer">✕ إغلاق</button>' +
        renderCartPanel(renderHomeOnly) +
        '</div>' +
        '</div>';
}
...
function renderStock() {
    ...
    var statsBar = '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:9px;margin-bottom:18px">' + [{
            ...
            l: "الأصناف",
            v: S.stock.length,
            g: "linear-gradient(135deg,#0d47a1,#0277bd)"
        },
        {
            ic: "🏪",
            l: "قيمة المحل",
            v: H(fmt(totalShop) + " دج"),
            g: "linear-gradient(135deg,#1b5e20,#2e7d32)"
        },
        {
            ic: "📦",
            l: "قيمة المستودع",
            v: H(fmt(totalWh) + " دج"),
            g: "linear-gradient(135deg,#4a148c,#6a1b9a)"
        },
        {
            ic: "⚠️",
            l: "ناقصة",
            v: lowCount,
            g: lowCount > 0 ? "linear-gradient(135deg,#bf360c,#e64a19)" : "linear-gradient(135deg,#1b5e20,#2e7d32)"
        }
    ].map(function(s) {
        return '<div style="background:' + s.g + ';border-radius:15px;padding:14px 9px;text-align:center;box-shadow:0 4px 14px rgba(0,0,0,.22)"><div style="font-size:22px;margin-bottom:4px">' + s.ic + '</div><div style="color:rgba(255,255,255,.75);font-size:11px;font-weight:600;margin-bottom:2px">' + s.l + '</div><div style="color:#fff;font-weight:900;font-size:14px">' + s.v + '</div></div>';
    }).join('') + '</div>';

    // فورم الإضافة ظاهر دائماً
    var formHTML = '<div style="background:linear-gradient(135deg,#e8f4fd,#f0f8ff);border:2px solid #b3d9f7;border-radius:16px;padding:18px;margin-bottom:16px;box-shadow:0 3px 12px rgba(2,119,189,.1)">' +
        ...
        '<select class="inp" onchange="S.form.cat=this.value">' + CATS.filter(function(x) {
            return x !== "الكل";
        }).map(function(c) {
    ...
    var toolBar = '<div style="background:#f8faff;border-radius:14px;padding:13px;margin-bottom:14px;border:2px solid #e8eeff;display:flex;flex-wrap:wrap;gap:9px;align-items:center">' +
        ...
        '<div style="display:flex;gap:4px;flex-wrap:wrap">' + sorts.map(function(k, i) {
            return '<button onclick="S.sortBy=\'' + k + '\';render()" style="padding:8px 11px;border-radius:9px;border:none;cursor:pointer;font-weight:700;font-size:13px;font-family:Tajawal,Arial;background:' + (S.sortBy === k ? 'linear-gradient(135deg,#0277bd,#01579b)' : '#eef1ff') + ';color:' + (S.sortBy === k ? '#fff' : '#555') + '">' + sortLabels[i] + '</button>';
        }).join('') + '</div>' +
        '<div style="display:flex;gap:4px">' + [
            ...
            return '<button onclick="S.stockView=\'' + v[0] + '\';render()" style="padding:8px 12px;border-radius:9px;border:none;cursor:pointer;font-size:15px;background:' + (S.stockView === v[0] ? 'linear-gradient(135deg,#0277bd,#01579b)' : '#eef1ff') + ';transition:all .15s">' + v[1] + '</button>';
        }).join('') + '</div>' +
        '</div>';

    var catTables = CATS.filter(function(x) {
        return x !== "الكل";
    }).map(function(cat) {
        ...
        var cv = items.reduce(function(a, s) {
            return a + s.q * s.c;
        }, 0);
        var wv = items.reduce(function(a, s) {
            return a + (s.wq || 0) * s.c;
        }, 0);
        var profit = items.reduce(function(a, s) {
            return a + s.q * (s.p - s.c);
        }, 0);
        var lowC = items.filter(function(s) {
            return s.q <= 5;
        }).length;
        ...
        var tbody = items.map(function(s, idx) {
            ...
            return '<tr style="background:' + rb + '" onmouseover="this.style.background=\'#f0f4ff\'" onmouseout="this.style.background=\'' + rb + '\'">' +
                (S.stockView !== "warehouse" ? '<td style="padding:8px 6px;border-bottom:1px solid #f0f0f0;text-align:center">' +
                '<input type="number" value="' + s.q + '" onchange="var it=S.stock.find(function(x){return x.id===' + s.id + '});if(it){it.q=+this.value;save();}" style="width:54px;padding:5px 3px;border-radius:8px;border:2px solid ' + (s.q === 0 ? '#f44336' : s.q <= 5 ? '#ff9800' : '#2e7d32') + ';text-align:center;font-size:14px;color:' + qc + ';font-weight:800;font-family:Tajawal,Arial;background:' + (s.q === 0 ? '#fff5f5' : s.q <= 5 ? '#fffde7' : '#f1f8e9') + '">' +
                '</td>' : '') +
                (S.stockView !== "shop" ? '<td style="padding:8px 6px;border-bottom:1px solid #f0f0f0;text-align:center">' +
                '<input type="number" value="' + (s.wq || 0) + '" onchange="var it=S.stock.find(function(x){return x.id===' + s.id + '});if(it){it.wq=+this.value;save();}" style="width:54px;padding:5px 3px;border-radius:8px;border:2px solid #7b1fa2;text-align:center;font-size:14px;color:#4a148c;font-weight:800;font-family:Tajawal,Arial;background:#f3e5f5">' +
                '</td>' : '') +
...
function renderPrint() {
    ...
    Object.keys(PI).forEach(function(t) {
        byType[t] = S.pjobs.filter(function(p) {
            return p.type === t;
        }).reduce(function(a, p) {
            return a + p.price;
        }, 0);
    });
    var maxVal = Math.max.apply(null, Object.keys(byType).map(function(k) {
        return byType[k] || 0;
    })) || 1;
    var statsBar = '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:18px">' + [{
            ...
            l: "اليوم",
            v: H(fmt(tPT()) + " دج"),
            g: "linear-gradient(135deg,#004d40,#00695c)"
        },
        {
            ic: "📅",
            l: "الشهر",
            v: H(fmt(mPT()) + " دج"),
            g: "linear-gradient(135deg,#00695c,#00897b)"
        },
        {
            ic: "📋",
            l: "إجمالي",
            v: S.pjobs.length,
            g: "linear-gradient(135deg,#006064,#00838f)"
        }
    ].map(function(s) {
        return '<div style="background:' + s.g + ';border-radius:15px;padding:16px 10px;text-align:center;box-shadow:0 4px 14px rgba(0,105,92,.4)"><div style="font-size:24px;margin-bottom:5px">' + s.ic + '</div><div style="color:rgba(255,255,255,.75);font-size:12px;font-weight:600;margin-bottom:4px">' + s.l + '</div><div style="color:#fff;font-weight:900;font-size:17px">' + s.v + '</div></div>';
    }).join('') + '</div>';
...
function renderClients() {
    ...
    return '<h3 style="margin:0 0 16px;color:#4a148c;font-size:22px;font-weight:900">📋 سجل الديون</h3>' +
        ...
        '<div><div style="color:rgba(255,255,255,.78);font-size:14px;font-weight:600">المدينون</div><div style="color:#fff;font-weight:900;font-size:26px">' + S.clients.filter(function(c) {
            return c.debt > 0;
        }).length + ' زبون</div></div>' +
        '</div>' +
...
function renderExpenses() {
    ...
    return '<div style="background:linear-gradient(135deg,#fff8e1,#fff3e0);border-radius:15px;padding:17px;margin-bottom:16px;border:2px solid #ffe082">' +
        ...
        '<div class="grid2"><input class="inp" value="' + esc(S.eForm.lbl) + '" onchange="S.eForm.lbl=this.value" placeholder="وصف المصروف"><select class="inp" onchange="S.eForm.cat=this.value">' + ECATS.map(function(c) {
            return '<option value="' + c + '"' + (S.eForm.cat === c ? ' selected' : '') + '>' + c + '</option>';
        }).join('') + '</select><input class="inp" type="number" value="' + esc(S.eForm.amt) + '" onchange="S.eForm.amt=this.value" placeholder="المبلغ (دج)" style="grid-column:span 2;border-color:#ff8a65"></div>' +
...
function renderReport() {
    ...
    var rT = rs.reduce(function(a, s) {
        return a + s.total;
    }, 0);
    var rP = rs.reduce(function(a, s) {
        return a + s.profit;
    }, 0);
    var rPR = rp.reduce(function(a, p) {
        return a + p.price;
    }, 0);
    var rFR = rf.reduce(function(a, f) {
        return a + (f.profit || 0);
    }, 0);
    var rE = re.reduce(function(a, e) {
        return a + e.amount;
    }, 0);
    var rNet = rP + rPR + rFR - rE;
    var repBtns = '<div style="display:flex;gap:6px;flex-wrap:wrap">' + [
        ["today", "اليوم"],
        ["week", "الأسبوع"],
        ["month", "الشهر"],
        ["all", "الكل"]
    ].map(function(p) {
        return '<button onclick="S.rep=\'' + p[0] + '\';render()" class="btn" style="background:' + (S.rep === p[0] ? 'linear-gradient(135deg,#0277bd,#01579b)' : '#f0f4ff') + ';color:' + (S.rep === p[0] ? '#fff' : '#0277bd') + ';border:2px solid ' + (S.rep === p[0] ? 'transparent' : '#dde4ff') + ';padding:9px 16px;font-size:15px">' + p[1] + '</button>';
    }).join('') + '</div>';
    ...
    var summCards = [{
        ...
        l: "Flixy",
        v: fmt(rFR) + " دج",
        g: "linear-gradient(135deg,#1a237e,#3949ab)"
    }, {
        ic: "💸",
        l: "مصاريف",
        v: fmt(rE) + " دج",
        g: "linear-gradient(135deg,#bf360c,#e64a19)"
    }, {
        ic: "🏆",
        l: "صافي الربح",
        v: fmt(rNet) + " دج",
        g: rNet >= 0 ? "linear-gradient(135deg,#1b5e20,#43a047)" : "linear-gradient(135deg,#b71c1c,#e53935)"
    }];
    ...
    var cloudSection = '<div style="background:linear-gradient(135deg,#1a2535,#0d1b2a);border-radius:18px;padding:22px;margin-bottom:20px;border:1px solid rgba(255,255,255,.1);box-shadow:0 6px 22px rgba(0,0,0,.3)">' +
        ...
        '<div><div style="font-size:16px;font-weight:800">رفع للسحابة</div><div style="font-size:12px;opacity:.8;margin-top:3px">رفع بيانات جهازك للسحابة</div></div>' +
        '</button>' +
        ...
        '<span style="font-size:30px">⬇️</span>' +
        '<div><div style="font-size:16px;font-weight:800">استيراد من السحابة</div><div style="font-size:12px;opacity:.8;margin-top:3px">تحميل بيانات السحابة لجهازك</div></div>' +
        '</button>' +
        '</div>' +
    ...
    var salesList = rs.slice().reverse().map(function(s, idx) {
        return '<div style="border:2px solid #e8eeff;border-radius:14px;padding:13px 16px;margin-bottom:11px;box-shadow:0 2px 10px rgba(0,0,0,.06)">' +
            ...
            s.items.map(function(it) {
                return '<div style="border-bottom:1px dashed #eee;display:flex;justify-content:space-between;padding:5px 0;font-size:13px"><span style="color:#555">' + esc(it.n) + ' × ' + it.q + '</span><span style="font-weight:700;color:#0277bd">' + it.sum + ' دج</span></div>';
            }).join('') +
...
function bindReportEvents() {
    ...
    if (upBtn) {
        upBtn.onclick = function() {
            ...
            pushToCloud();
            setTimeout(function() {
                statusEl.innerHTML = '<span style="color:#69f0ae;font-weight:700">✅ تم رفع البيانات بنجاح!</span>';
            }, 1200);
        };
    }
    if (downBtn) {
        downBtn.onclick = function() {
            ...
            sbGet("app_state", function(data) {
                if (!data || !data.stock || data.stock.length === 0) {
                    statusEl.innerHTML = '<span style="color:#ef9a9a;font-weight:700">❌ السحابة فارغة!</span>';
                    return;
                }
                applyCloud(data);
...
function printReport() {
    ...
    var html = '<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><title>تقرير</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Tajawal,Arial,sans-serif;font-size:13px;direction:rtl;padding:10mm;color:#000}@media print{@page{size:A4;margin:10mm}}.title{text-align:center;font-size:22px;font-weight:900;margin-bottom:4px}.sub{text-align:center;font-size:13px;color:#555;margin-bottom:14px}table{width:100%;border-collapse:collapse;margin-bottom:14px}th{background:#0277bd;color:#fff;padding:8px;font-size:13px}td{padding:6px 8px;border:1px solid #ddd;font-size:12px}.summary{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:14px}.box{border:2px solid #0277bd;border-radius:10px;padding:12px;text-align:center}.box-label{font-size:11px;color:#666}.box-val{font-size:17px;font-weight:bold;color:#0277bd}.net{font-size:22px;font-weight:bold;text-align:center;padding:12px;border:2px solid;border-radius:10px;margin-bottom:14px}</style></head><body>' +
        '<div class="title">📚 مكتبة حشايشي</div><div class="sub">تقرير: ' + periods[S.rep] + ' | ' + new Date().toLocaleDateString("ar-DZ", {
            year: "numeric",
            month: "long",
            day: "numeric"
        }) + '</div>' +
        ...
        (rs.length > 0 ? '<table><thead><tr><th>#</th><th>الوقت</th><th>الأصناف</th><th>الإجمالي</th><th>الربح</th></tr></thead><tbody>' + rs.slice().reverse().map(function(s, i) {
