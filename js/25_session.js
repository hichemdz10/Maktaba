...
function doAutoBackup(){
  try{
    // نسخة احتياطية مضغوطة: بدون الصور لتجنب QuotaExceededError
    var dbLight={
      products: db.products.map(function(p){
        var c=Object.assign({},p); delete c.image; return c;
      }),
      sales:           db.sales,
      returns:         db.returns,
      ...
      telecomSales:    db.telecomSales,
      printSales:      db.printSales,
      suppliers:       db.suppliers||[],
      supplierPayments:db.supplierPayments||[],
      employees:       db.employees||[],
    };
    var snap=JSON.stringify({date:nowStr(),db:dbLight});
    // تنظيف النسخة القديمة أولاً لتحرير المساحة
    try{localStorage.removeItem('hch_auto_backup');}catch(e){}
    localStorage.setItem('hch_auto_backup',snap);
    var el=$('backup-dot');
    if(el){el.style.opacity='1';setTimeout(function(){el.style.opacity='0';},2000);}
  }catch(e){
    // إذا امتلأ localStorage اعرض تحذيراً مرة واحدة فقط
    if(e.name==='QuotaExceededError'&&!doAutoBackup._warned){
      doAutoBackup._warned=true;
      toast('⚠️ مساحة المتصفح ممتلئة — لن تُحفظ نسخة تلقائية. استخدم تصدير النسخ الاحتياطية يدوياً.','error',6000);
    }
    console.warn('Auto-backup failed:',e.name);
  }
}
doAutoBackup._warned=false;
