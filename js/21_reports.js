...
function openSaleConfirm(){
  ...
  var disc=parseFloat(($('disc-in')||{}).value)||parseFloat(($('disc-in-normal')||{}).value)||0;
  var total=Math.max(0,sub-disc);
  var method=($('pay-method')||{}).value||'cash';
  var mm={cash:'💵 نقداً',card:'💳 بطاقة',debt:'📝 دين',partial:'💰 دفع جزئي'};
  var isPro=_isProLayout();
  var html='';
  ...
  // طريقة الدفع + اسم العميل
  var methodLabel=mm[method]||method;
  var clientFld=isPro?$('sale-client'):$('sale-client-n');
  var pClieFld=isPro?$('partial-client'):$('partial-client-n');
  var paidFld=isPro?$('partial-paid'):$('partial-paid-n');
  if(method==='debt'&&clientFld&&clientFld.value){
    var opt=clientFld.options[clientFld.selectedIndex];
    if(opt&&opt.text)methodLabel+=' — '+opt.text.split(' —')[0];
  }
  if(method==='partial'){
    var paidNow=parseFloat((paidFld||{}).value)||0;
    if(pClieFld&&pClieFld.value){var opt2=pClieFld.options[pClieFld.selectedIndex];if(opt2&&opt2.text)methodLabel+=' — '+opt2.text.split(' —')[0];}
    if(paidNow>0)methodLabel+=' ('+fmt(paidNow)+' الآن، متبقي '+fmt(Math.max(0,total-paidNow))+')';
  }
  if($('sc-method'))$('sc-method').textContent=methodLabel;
  var dr=$('sc-disc-row');
