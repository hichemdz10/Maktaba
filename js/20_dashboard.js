...
async function checkout(){
  ...
  // قراءة الخصم من أي من الحقلين
  var disc = Math.max(0, parseFloat(($('disc-in')||{}).value) || parseFloat(($('disc-in-normal')||{}).value) || 0);
  ...
  // ── تحديد مصدر البيانات: العادي أم الاحترافي
  var isPro = _isProLayout();
  var givenFld    = isPro ? $('cash-given')    : $('cash-given-n');
  ...
  if(method==='partial'){
    ...
    var partialClientId = (pClieFld||{}).value;
  ...
  if(method==='cash'){ sale.givenAmount = parseFloat((givenFld||{}).value)||0; }
  if(method==='debt'){
    var clientId = (clientFld||{}).value;
